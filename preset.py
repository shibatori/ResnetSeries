...
'''
df_test = pd.read_csv("unsw_test.csv")  # 82332
df_train = pd.read_csv("unsw_train.csv")
...
#打乱数据集
index = [i for i in range(0, int(data_train.shape[0]))]
random.shuffle(index)
data_train = data_train[index]
# label_train = label_train[index]
attack_cat_train = attack_cat_train[index]

index = [i for i in range(0, int(data_test.shape[0]))]
random.shuffle(index)
data_test = data_test[index]
# label_test = label_test[index]
attack_cat_test = attack_cat_test[index]
...
np.savez("./UNSWNB_labelall_train_dataset.npz",data=data_train,label=attack_cat_train,attack_cat_examples=attack_cat_examples)
np.savez("./UNSWNB_labelall_test_dataset.npz",data=data_test,label=attack_cat_test,attack_cat_examples=attack_cat_examples)

# dataset_train=np.load("./UNSWNB_labelall_train_dataset.npz")
# x_train,y_train=dataset_train["data"],dataset_train["label"]
# dataset_test=np.load("./UNSWNB_labelall_test_dataset.npz")
